...
# NOTE: Import paths updated for HyperFreeBackbone standalone reuse; all encoder logic
# stays identical to the official HyperFree implementation so pretrained weights align.
...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor, test_mode=False, input_wavelength=None, GSD=None) -> torch.Tensor:
        ...
        if not test_mode:
            # This is the dangerous branch with random bands
            print("[WARNING] ImageEncoderViT forward called with test_mode=False → RANDOM BAND SAMPLING ACTIVE!")
        else:
            # You can log once to be safe
            # print("[DEBUG] ImageEncoderViT forward uses deterministic band selection (no random sampling).")
            pass
