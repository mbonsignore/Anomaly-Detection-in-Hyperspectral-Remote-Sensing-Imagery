#!/usr/bin/env python3
import os
from pathlib import Path
from typing import Tuple, List
import sys
import csv

import numpy as np
import torch
import rasterio
import torch.nn.functional as F
import matplotlib.pyplot as plt
import random

# Absolute path to HyperFree project root
PROJECT_ROOT = Path("../../../home/mbonsignore/HyperfreeBackbone-ActiveFireNoThermal/HyperFreeBackbone")

if not PROJECT_ROOT.exists():
    raise RuntimeError(f"PROJECT_ROOT does not exist: {PROJECT_ROOT}")

if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from config import BackboneConfig
from model import build_image_encoder, load_weights

# ================================
# CONFIG
# ================================

BASE_DIR = Path(__file__).resolve().parent

# Root directory with the three S2–PRISMA acquisition folders
ROOT_DIR = BASE_DIR / "Acquisizioni_S2_PRISMA"

# HyperFree checkpoint (relative or absolute)
HYPERFREE_CKPT = PROJECT_ROOT / "ckpt/HyperFree-b.pth"

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Sentinel-2: all bands resampled to 10m
GSD_S2 = 10.0
# PRISMA: typical ~30m (adjust if you know the exact value)
GSD_PRISMA = 30.0

# Sentinel-2 central wavelengths in nm, in order:
# B2, B3, B4, B5, B6, B7, B8, B8A, B11, B12
S2_WAVELENGTHS_NM = np.array([
    492.4,   # B02 (blue)
    559.8,   # B03 (green)
    664.6,   # B04 (red)
    704.1,   # B05 (red edge)
    740.5,   # B06 (red edge)
    782.8,   # B07 (red edge)
    832.8,   # B08 (NIR)
    864.7,   # B8A (NIR)
    1613.7,  # B11 (SWIR)
    2202.4,  # B12 (SWIR)
], dtype=np.float32)


# ================================
# UTILS
# ================================

def set_global_seeds(seed: int = 42):
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(seed)

    # PyTorch reproducibility flags
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False

def get_hyperfree_encoder() -> Tuple[torch.nn.Module, int]:
    """
    Build HyperFree image encoder and return (encoder, img_size).
    Uses the same config API you used in finetuning.
    """
    config = BackboneConfig(dataset_type="floga")  # or your preferred config
    img_size = config.img_size

    encoder = build_image_encoder(config)
    encoder = load_weights(encoder, HYPERFREE_CKPT)
    encoder.to(DEVICE)
    encoder.eval()
    encoder.requires_grad_(False)
    return encoder, img_size


def get_embedding_from_feature_map(features: torch.Tensor) -> torch.Tensor:
    """
    Average over spatial tokens of the last feature map [B,C,H,W] → [B,D].
    """
    if isinstance(features, (list, tuple)):
        features = features[-1]
    if features.dim() != 4:
        raise ValueError(f"Expected [B,C,H,W], got {tuple(features.shape)}")
    return features.flatten(2).mean(dim=2)  # [B,D]


def load_sentinel2_reflectance(path: Path) -> np.ndarray:
    """
    Load sentinel_mosaic.tif and convert DN -> reflectance.
    Returns array [C,H,W] in float32, clipped to [0,1].
    """
    with rasterio.open(path) as ds:
        # rasterio reads [bands, H, W] by default
        dn = ds.read().astype(np.float32)

    refl = (dn - 1000.0) / 10000.0
    refl = np.clip(refl, 0.0, 1.0).astype(np.float32)
    return refl  # [C,H,W]


def load_prisma_reflectance_and_wavelengths(path: Path) -> Tuple[np.ndarray, np.ndarray]:
    """
    Load PRISMA reflectance_coregistered.tif and extract wavelengths from metadata.

    Returns:
        img: [C,H,W] float32, clipped to [0,1]
        wavelengths_nm: [C] float32, central wavelength per band in nm
    """
    with rasterio.open(path) as ds:
        img = ds.read().astype(np.float32)  # [C,H,W]

        # Clip to [0,1] just to be safe
        img = np.clip(img, 0.0, 1.0).astype(np.float32)

        # --- Try to read wavelengths from metadata ---
        tags_global = ds.tags()
        wavelengths_nm = None

        # 1) Global tag containing all wavelengths
        for key in ["WAVELENGTH", "wavelength", "Wavelength"]:
            if key in tags_global:
                raw = tags_global[key]
                # often comma- or space-separated
                parts = [p for p in raw.replace(";", ",").replace(" ", ",").split(",") if p]
                wavelengths_nm = np.array([float(p) for p in parts], dtype=np.float32)
                break

        # 2) If not found, try per-band tags
        if wavelengths_nm is None:
            wl_list: List[float] = []
            for bidx in range(1, ds.count + 1):
                band_tags = ds.tags(bidx=bidx)
                if "WAVELENGTH" in band_tags:
                    wl_list.append(float(band_tags["WAVELENGTH"]))
                elif "wavelength" in band_tags:
                    wl_list.append(float(band_tags["wavelength"]))
                else:
                    # First time we fail, print some info and raise
                    print(f"[WARN] No wavelength key found for band {bidx}. "
                          f"Available keys: {list(band_tags.keys())}")
                    raise RuntimeError(
                        f"Cannot find wavelength metadata for PRISMA file {path}"
                    )
            wavelengths_nm = np.array(wl_list, dtype=np.float32)

        if wavelengths_nm.shape[0] != img.shape[0]:
            raise RuntimeError(
                f"Mismatch between bands in image ({img.shape[0]}) "
                f"and wavelengths ({wavelengths_nm.shape[0]}) in {path}"
            )

    return img, wavelengths_nm


def load_s2_synthetic(path: Path) -> np.ndarray:
    """
    Load synthetic S2-from-PRISMA reflectance (already in [0,1]).
    Returns [C,H,W] float32, clipped to [0,1].
    """
    with rasterio.open(path) as ds:
        img = ds.read().astype(np.float32)  # [C,H,W]
    img = np.clip(img, 0.0, 1.0).astype(np.float32)
    return img


def find_s2_synth_file(prisma_path: Path) -> Path:
    """
    Given the PRISMA reflectance_coregistered.tif path, return the expected
    synthetic S2 file path produced by the PRISMA→S2 SRF conversion script.
    """
    synth_path = prisma_path.with_name(prisma_path.stem + "_S2synth.tif")
    if not synth_path.is_file():
        raise RuntimeError(
            f"Synthetic S2 file not found: {synth_path}\n"
            "Make sure you have run the PRISMA→S2 SRF conversion script."
        )
    return synth_path


def prepare_tensor_for_hyperfree(
    img_chw: np.ndarray,
    img_size: int,
) -> torch.Tensor:
    """
    Take a [C,H,W] numpy array in [0,1], resize to [C,img_size,img_size]
    and return a [1,C,img_size,img_size] float32 torch tensor on DEVICE.
    """
    tensor = torch.from_numpy(img_chw).unsqueeze(0)  # [1,C,H,W]
    tensor = tensor.to(DEVICE)

    _, _, H, W = tensor.shape
    if H != img_size or W != img_size:
        tensor = F.interpolate(
            tensor,
            size=(img_size, img_size),
            mode="bilinear",
            align_corners=False,
        )
    return tensor  # [1,C,img_size,img_size]


def encode_image(
    encoder: torch.nn.Module,
    img_chw: np.ndarray,
    wavelengths_nm: np.ndarray,
    img_size: int,
    gsd_meters: float,
) -> torch.Tensor:
    """
    Run HyperFree encoder on a single image [C,H,W] with associated wavelengths and GSD.
    Returns embedding vector [D] (torch.float32 on DEVICE).
    """
    assert img_chw.ndim == 3
    assert img_chw.shape[0] == wavelengths_nm.shape[0], \
        "Number of bands must match number of wavelengths."

    tensor = prepare_tensor_for_hyperfree(img_chw, img_size)  # [1,C,H,W]

    wavelengths_list = wavelengths_nm.astype(float).tolist()
    gsd_list = [float(gsd_meters)]

    with torch.no_grad(), torch.cuda.amp.autocast(enabled=(DEVICE.type == "cuda")):
        feats = encoder(
            tensor,
            test_mode=True,
            input_wavelength=wavelengths_list,
            GSD=gsd_list,
        )
        z = get_embedding_from_feature_map(feats)  # [1,D]

    return z.squeeze(0)  # [D]


def find_pair_files(folder: Path) -> Tuple[Path, Path]:
    """
    Given a folder for a single acquisition, find:
      - sentinel_mosaic*.tif  (real Sentinel-2 mosaic)
      - *reflectance_coregistered.tif (real PRISMA cube, NOT the synthetic S2)
    """
    s2 = None
    prisma = None

    for tif in folder.glob("*.tif"):
        name = tif.name.lower()

        # Real Sentinel-2 mosaic
        if "sentinel_mosaic" in name:
            s2 = tif

        # Real PRISMA reflectance cube:
        # must have "reflectance_coregistered" but NOT "s2synth"
        elif "reflectance_coregistered" in name and "s2synth" not in name:
            prisma = tif

    if s2 is None or prisma is None:
        raise RuntimeError(
            f"Could not find both real sentinel_mosaic and real *reflectance_coregistered in {folder}\n"
            f"  found S2: {s2}\n"
            f"  found PRISMA: {prisma}"
        )

    return s2, prisma


def cosine_similarity(z1: torch.Tensor, z2: torch.Tensor) -> float:
    z1n = z1 / (z1.norm(p=2) + 1e-8)
    z2n = z2 / (z2.norm(p=2) + 1e-8)
    return float(torch.dot(z1n, z2n).item())


# ================================
# MAIN
# ================================

def main():
    set_global_seeds(42)
    encoder, img_size = get_hyperfree_encoder()
    print(f"[INFO] Using HyperFree encoder with img_size={img_size}, device={DEVICE}")

    if not ROOT_DIR.is_dir():
        raise RuntimeError(f"Root directory {ROOT_DIR} does not exist.")

    acquisition_folders = sorted(
        [p for p in ROOT_DIR.iterdir() if p.is_dir()]
    )

    if not acquisition_folders:
        raise RuntimeError(f"No acquisition folders found under {ROOT_DIR}")

    print(f"[INFO] Found {len(acquisition_folders)} acquisition folders under {ROOT_DIR.name}.")

    # To store all results for CSV and plots
    all_results: List[dict] = []

    for folder in acquisition_folders:
        print(f"\n=== Acquisition: {folder.name} ===")

        s2_path, prisma_path = find_pair_files(folder)
        print(f"[INFO] Sentinel-2 file: {s2_path.name}")
        print(f"[INFO] PRISMA file:      {prisma_path.name}")

        # Synthetic S2 from PRISMA (produced by the conversion script)
        s2_synth_path = find_s2_synth_file(prisma_path)
        print(f"[INFO] S2-synth file:    {s2_synth_path.name}")

        # ---- Load real Sentinel-2 ----
        s2_img = load_sentinel2_reflectance(s2_path)  # [C,H,W]
        s2_wl = S2_WAVELENGTHS_NM

        if s2_img.shape[0] != len(s2_wl):
            raise RuntimeError(
                f"Sentinel-2 band count ({s2_img.shape[0]}) does not match "
                f"wavelengths ({len(s2_wl)})"
            )

        # ---- Load synthetic Sentinel-2 (same spectral bands as real S2) ----
        s2_synth_img = load_s2_synthetic(s2_synth_path)  # [10,H,W]
        if s2_synth_img.shape[0] != len(s2_wl):
            raise RuntimeError(
                f"S2-synth band count ({s2_synth_img.shape[0]}) does not match "
                f"wavelengths ({len(s2_wl)})"
            )

        # ---- Load PRISMA ----
        prisma_img, prisma_wl = load_prisma_reflectance_and_wavelengths(prisma_path)
        print(f"[INFO] PRISMA bands: {prisma_img.shape[0]}")

        # ---- Encode all three with HyperFree ----
        z_s2_real = encode_image(
            encoder=encoder,
            img_chw=s2_img,
            wavelengths_nm=s2_wl,
            img_size=img_size,
            gsd_meters=GSD_S2,
        )

        # Synthetic S2 keeps PRISMA spatial resolution (~30m),
        # but uses Sentinel-2 spectral bands:
        z_s2_synth = encode_image(
            encoder=encoder,
            img_chw=s2_synth_img,
            wavelengths_nm=s2_wl,
            img_size=img_size,
            gsd_meters=GSD_PRISMA,
        )

        z_prisma = encode_image(
            encoder=encoder,
            img_chw=prisma_img,
            wavelengths_nm=prisma_wl,
            img_size=img_size,
            gsd_meters=GSD_PRISMA,
        )

        # ---- Compute pairwise distances & similarities ----
        def l2(a: torch.Tensor, b: torch.Tensor) -> float:
            return float(torch.norm(a - b, p=2).item())

        cos_prisma_s2_real   = cosine_similarity(z_prisma,     z_s2_real)
        cos_prisma_s2_synth  = cosine_similarity(z_prisma,     z_s2_synth)
        cos_s2_real_s2_synth = cosine_similarity(z_s2_real,    z_s2_synth)

        # cosine distances = 1 - cosine similarity
        cosd_prisma_s2_real   = 1.0 - cos_prisma_s2_real
        cosd_prisma_s2_synth  = 1.0 - cos_prisma_s2_synth
        cosd_s2_real_s2_synth = 1.0 - cos_s2_real_s2_synth

        l2_prisma_s2_real   = l2(z_prisma,  z_s2_real)
        l2_prisma_s2_synth  = l2(z_prisma,  z_s2_synth)
        l2_s2_real_s2_synth = l2(z_s2_real, z_s2_synth)

        print("\n[RESULT] Cosine similarities:")
        print(f"  PRISMA vs S2 (real):       {cos_prisma_s2_real:.4f}")
        print(f"  PRISMA vs S2 (synth):      {cos_prisma_s2_synth:.4f}")
        print(f"  S2 (real) vs S2 (synth):   {cos_s2_real_s2_synth:.4f}")

        print("\n[RESULT] Cosine distances (1 - cos):")
        print(f"  PRISMA vs S2 (real):       {cosd_prisma_s2_real:.4f}")
        print(f"  PRISMA vs S2 (synth):      {cosd_prisma_s2_synth:.4f}")
        print(f"  S2 (real) vs S2 (synth):   {cosd_s2_real_s2_synth:.4f}")

        print("\n[RESULT] L2 distances:")
        print(f"  PRISMA vs S2 (real):       {l2_prisma_s2_real:.4f}")
        print(f"  PRISMA vs S2 (synth):      {l2_prisma_s2_synth:.4f}")
        print(f"  S2 (real) vs S2 (synth):   {l2_s2_real_s2_synth:.4f}")

        # Store everything for CSV & plots
        all_results.append({
            "acquisition": folder.name,
            "cos_prisma_s2_real":        cos_prisma_s2_real,
            "cos_prisma_s2_synth":       cos_prisma_s2_synth,
            "cos_s2_real_s2_synth":      cos_s2_real_s2_synth,
            "cosdist_prisma_s2_real":    cosd_prisma_s2_real,
            "cosdist_prisma_s2_synth":   cosd_prisma_s2_synth,
            "cosdist_s2_real_s2_synth":  cosd_s2_real_s2_synth,
            "l2_prisma_s2_real":         l2_prisma_s2_real,
            "l2_prisma_s2_synth":        l2_prisma_s2_synth,
            "l2_s2_real_s2_synth":       l2_s2_real_s2_synth,
        })

    # ================================
    # SAVE CSV + PLOTS
    # ================================
    if not all_results:
        print("[WARN] No results collected, skipping CSV/plots.")
        return

    # ---- CSV ----
    csv_path = BASE_DIR / "prisma_s2_hyperfree_comparison.csv"
    fieldnames = [
        "acquisition",
        "cos_prisma_s2_real",
        "cos_prisma_s2_synth",
        "cos_s2_real_s2_synth",
        "cosdist_prisma_s2_real",
        "cosdist_prisma_s2_synth",
        "cosdist_s2_real_s2_synth",
        "l2_prisma_s2_real",
        "l2_prisma_s2_synth",
        "l2_s2_real_s2_synth",
    ]
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        for row in all_results:
            writer.writerow(row)

    print(f"\n[INFO] Saved CSV results to: {csv_path}")

    # ---- Plots ----
    plots_dir = BASE_DIR / "plots"
    plots_dir.mkdir(exist_ok=True)

    acq_names = [r["acquisition"] for r in all_results]
    x = np.arange(len(acq_names))

    # Cosine similarities plot
    width = 0.25
    plt.figure(figsize=(8, 5))
    plt.bar(x - width, [r["cos_prisma_s2_real"] for r in all_results],
            width, label="PRISMA–S2 (real)")
    plt.bar(x,        [r["cos_prisma_s2_synth"] for r in all_results],
            width, label="PRISMA–S2 (synth)")
    plt.bar(x + width, [r["cos_s2_real_s2_synth"] for r in all_results],
            width, label="S2 (real)–S2 (synth)")

    plt.xticks(x, acq_names, rotation=45)
    plt.ylim(0.0, 1.0)
    plt.ylabel("Cosine similarity")
    plt.title("HyperFree embedding cosine similarities")
    plt.legend()
    plt.tight_layout()
    cos_plot_path = plots_dir / "cosine_similarities.png"
    plt.savefig(cos_plot_path, dpi=300)
    plt.close()
    print(f"[INFO] Saved cosine similarity plot to: {cos_plot_path}")

    # L2 distances plot
    plt.figure(figsize=(8, 5))
    plt.bar(x - width, [r["l2_prisma_s2_real"] for r in all_results],
            width, label="PRISMA–S2 (real)")
    plt.bar(x,        [r["l2_prisma_s2_synth"] for r in all_results],
            width, label="PRISMA–S2 (synth)")
    plt.bar(x + width, [r["l2_s2_real_s2_synth"] for r in all_results],
            width, label="S2 (real)–S2 (synth)")

    plt.xticks(x, acq_names, rotation=45)
    plt.ylabel("L2 distance")
    plt.title("HyperFree embedding L2 distances")
    plt.legend()
    plt.tight_layout()
    l2_plot_path = plots_dir / "l2_distances.png"
    plt.savefig(l2_plot_path, dpi=300)
    plt.close()
    print(f"[INFO] Saved L2 distance plot to: {l2_plot_path}")

    print("\n✅ Done: CSV and plots generated.")


if __name__ == "__main__":
    main()