#!/usr/bin/env python3
"""Semi-supervised fine-tuning of Copernicus-FM on FLOGA with
...
from config import BackboneConfig, build_config
from model import build_image_encoder
from utils.spectral_meta import resolve_wave_bandwidth

# ============================
#  IMPORT PATCHCORE MODULES
#  (robust search, avoid utils conflicts)
# ============================
from importlib.machinery import SourceFileLoader
import sys


def _find_patchcore_dir() -> Path:
    start = Path(__file__).resolve().parent
    for parent in [start] + list(start.parents):
        candidate = parent / "../../home/mbonsignore/patchcore"
        if (candidate / "utils.py").is_file() and (candidate / "memory_bank.py").is_file():
            return candidate
    fallback = (Path(__file__).resolve().parent / "../../patchcore").resolve()
    if (fallback / "utils.py").is_file() and (fallback / "memory_bank.py").is_file():
        return fallback
    raise FileNotFoundError("PatchCore directory not found (expected patchcore/utils.py and memory_bank.py)")


PATCHCORE_DIR = _find_patchcore_dir()
...
# Restore local utils module if it existed
...
class FlogaCsvDataset(Dataset):
    ...
    def __init__(
        ...
        self.cache_channels: int | None = None
    ...
    def _resolve_meta(self, channels: int) -> Tuple[torch.Tensor, torch.Tensor]:
        if (
            self.wave_cache is None
            or self.band_cache is None
            or self.cache_channels != channels
        ):
            wave, band = resolve_wave_bandwidth("floga", channels, None, None)
            self.wave_cache = wave
            self.band_cache = band
    ...
    def __getitem__(self, index: int) -> Dict[str, torch.Tensor | str]:
        ...
        if tensor.ndim != 3:
            print(f"[WARN] Invalid shape for {path}: {tuple(tensor.shape)}")
            return self.__getitem__((index + 1) % len(self.samples))
...
def parse_args() -> argparse.Namespace:
    ...
    default_data_root = base_dir / "../Datasets/FLOGA"
    default_splits_root = base_dir / "../Datasets/FLOGA/finetuning data/floga_splits"

    parser = argparse.ArgumentParser(
        description=(
            "Semi-supervised fine-tuning of Copernicus-FM backbone on FLOGA "
    ...
    parser.add_argument(
        "--teacher-ckpt",
        type=str,
        default=str(base_dir / "ckpt/CopernicusFM_ViT_base_varlang_e100.pth"),
    )
...
def get_feature_map(features: torch.Tensor | List[torch.Tensor]) -> torch.Tensor:
...
def get_embedding(features: torch.Tensor | List[torch.Tensor]) -> torch.Tensor:
    ...
    fmap = get_feature_map(features)
    return fmap.flatten(2).mean(dim=2)


def maybe_load_weights(encoder: torch.nn.Module, ckpt_path: Path) -> None:
    if not ckpt_path.exists():
        raise FileNotFoundError(f"Checkpoint not found: {ckpt_path}")
    if hasattr(encoder, "load_weights"):
...
def precompute_teacher_cache(
    ...
    with tqdm(loader, desc="Precomputing teacher embeddings (train)", unit="batch") as pbar:
        for batch in pbar:
            ...
            meta_info = batch["meta_info"].to(device, non_blocking=True)
            wavelengths = batch["wavelengths"].to(device, non_blocking=True)
            bandwidth = batch["bandwidth"].to(device, non_blocking=True)
            labels = batch["label"].cpu().numpy()  # [B]
            paths = batch["path"]  # list of str

            with torch.cuda.amp.autocast(enabled=use_amp):
                feats = teacher(images, meta_info, wavelengths, bandwidth)
...
def freeze_backbone(
    ...
    # Try to find blocks in the specified order
    if hasattr(encoder, "blocks") and isinstance(getattr(encoder, "blocks"), torch.nn.ModuleList):
        ...
        blocks_name = "encoder.blocks"
    elif hasattr(encoder, "backbone") and hasattr(encoder.backbone, "blocks"):
        blocks = encoder.backbone.blocks
        blocks_name = "backbone.blocks"
    elif hasattr(encoder, "transformer") and hasattr(encoder.transformer, "blocks"):
        blocks = encoder.transformer.blocks
        blocks_name = "transformer.blocks"
    elif hasattr(encoder, "model") and hasattr(encoder.model, "blocks"):
        blocks = encoder.model.blocks
        blocks_name = "model.blocks"
        ...
        if unfreeze_neck_norms:
            for name, module in encoder.named_modules():
                if isinstance(module, torch.nn.LayerNorm):
                    if "blocks." not in name and "encoder.blocks." not in name and "backbone.blocks." not in name and "transformer.blocks." not in name:
                        for param in module.parameters():
                            param.requires_grad = True

        print(f"[INFO] Unfroze last {unfreeze_blocks}/{num_blocks} blocks from {blocks_name}.")
    else:
        # Fallback: unfreeze parameters by name patterns
        for name, param in encoder.named_parameters():
            if (
                "blocks." in name
                or "encoder.blocks." in name
                or "norm" in name
                or "ln" in name
            ):
                param.requires_grad = True
        print("[WARN] Could not locate blocks ModuleList; using name-based unfreeze fallback.")


@torch.no_grad()
def log_first_batch_shapes(
    loader: DataLoader,
    encoder: torch.nn.Module,
    device: torch.device,
    use_amp: bool,
    ...
    for batch in loader:
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            feats = encoder(images, meta_info, wavelengths, bandwidth)
            fmap = get_feature_map(feats)

        print(f"[INFO] First batch input shape: {tuple(images.shape)}")
        print(f"[INFO] First batch feature map shape: {tuple(fmap.shape)}")
        break
...
def gradient_sanity_check(
    ...
    print("[INFO] Running gradient sanity check (single batch, full-loss formula).")
    ...
    for batch in loader:
        if batch is not None:
            break
    if batch is None:
        raise RuntimeError("Gradient sanity check failed: empty training loader.")
    ...
    # Ensure we always test full-loss gradient flow, even if training is margin-only
    lambda_center_eff = lambda_center
    lambda_distill_eff = lambda_distill
    if lambda_center_eff == 0.0 and lambda_distill_eff == 0.0:
        lambda_distill_eff = 1.0
        print(
            "[WARN] lambda_center and lambda_distill are 0; "
            "using lambda_distill=1.0 for gradient sanity check."
        )
    ...
    with torch.set_grad_enabled(True):
        with torch.cuda.amp.autocast(enabled=use_amp):
            z_teacher = torch.stack([teacher_cache[p] for p in paths]).to(device)  # [B,D]

            student_features = student(images, meta_info, wavelengths, bandwidth)
            ...
            if len(center_losses) > 0:
                ...
                loss_center_total = torch.tensor(0.0, device=device, dtype=z_student.dtype)

            diff_distill = z_student - z_teacher.to(z_student.dtype)
            loss_distill = diff_distill.pow(2).sum(dim=1).mean()

            loss = lambda_center_eff * loss_center_total + lambda_distill_eff * loss_distill

        loss.backward()

    trainable = [(n, p) for n, p in student.named_parameters() if p.requires_grad]
    params_with_grad = [(n, p) for n, p in trainable if p.grad is not None]
    params_with_nonzero = [
        (n, p) for n, p in params_with_grad if p.grad.detach().abs().sum().item() > 0.0
    ]

    print(f"[INFO] Trainable params: {len(trainable)}")
    print(f"[INFO] Params with grad: {len(params_with_grad)}")

    sample_norms = []
    for n, p in params_with_grad[:3]:
        grad_norm = float(p.grad.detach().norm().item())
        sample_norms.append((n, grad_norm))
    if sample_norms:
        for name, gnorm in sample_norms:
            print(f"[INFO] Grad norm sample: {name} -> {gnorm:.6e}")

    if len(params_with_nonzero) == 0:
        raise RuntimeError(
            "Gradient sanity check failed: no non-zero gradients on trainable parameters."
        )

    # Clean up grads before real training
    student.zero_grad(set_to_none=True)
...
def train_one_epoch(
    ...
    for batch in tqdm(
        ...
        if margin_only:
            ...
            meta_info_anom = batch["meta_info"][anom_mask_cpu].to(device, non_blocking=True)
            wavelengths_anom = batch["wavelengths"][anom_mask_cpu].to(device, non_blocking=True)
            bandwidth_anom = batch["bandwidth"][anom_mask_cpu].to(device, non_blocking=True)

            with torch.cuda.amp.autocast(enabled=use_amp):
                student_features = student(images_anom, meta_info_anom, wavelengths_anom, bandwidth_anom)
...
def run_training(args: argparse.Namespace, batch_size: int) -> None:
    ...
    config = build_config("floga")
    config.checkpoint_path = Path(args.teacher_ckpt)
    ...
    maybe_load_weights(teacher, Path(args.teacher_ckpt))
    teacher.to(device)
    teacher.eval()
    teacher.requires_grad_(False)

    student = build_image_encoder(config)
    maybe_load_weights(student, Path(args.teacher_ckpt))
    student.to(device)
    ...
    # Stats from VAL set
    val_labels = [lbl for _, _, lbl in val_dataset.samples]
    n_val_norm = sum(1 for l in val_labels if l == 0)
    n_val_anom = sum(1 for l in val_labels if l == 1)

    print(f"[INFO] Full train samples: {len(full_train_dataset)} "
          f"(normals={n_full_norm}, anomalies={n_full_anom})")
    print(f"[INFO] Val samples:   {len(val_dataset)} "
          f"(normals={n_val_norm}, anomalies={n_val_anom})")
    ...
    # Log first batch shapes (input + feature map)
    log_first_batch_shapes(full_train_loader, student, device=device, use_amp=use_amp)
    ...
    # Gradient sanity check (runs once before training loop)
    ...
    best_path = out_dir / "copernicus_floga_semisupervised_best.pt"
...
RUN 1 (safe):                           python3 train_finetune_copernicus_floga_semisupervised.py --data-root ../FLOGA --splits-root "../FLOGA/finetuning data/floga_splits" --teacher-ckpt ckpt/CopernicusFM_ViT_base_varlang_e100.pth --out-ckpt-dir ckpt --epochs 10 --batch-size 4 --min-batch-size 2 --lr 1e-4 --weight-decay 1e-4 --lambda-center 0.3 --lambda-distill 1.0 --lambda-anom 1.0 --anom-margin 0.9 --load-teacher-cache 

RUN 2 (aggressive):                     python3 train_finetune_copernicus_floga_semisupervised.py --data-root ../Datasets/FLOGA --splits-root "../Datasets/FLOGA/finetuning data/floga_splits" --teacher-ckpt ckpt/CopernicusFM_ViT_base_varlang_e100.pth --out-ckpt-dir ckpt --epochs 10 --batch-size 4 --min-batch-size 2 --lr 1e-4 --weight-decay 1e-4 --lambda-center 0.3 --lambda-distill 0.5 --lambda-anom 3.0 --anom-margin 1.3 --load-teacher-cache

RUN 3 (margin only equal to run 2):     python3 train_finetune_copernicus_floga_semisupervised.py --data-root ../Datasets/FLOGA --splits-root "../Datasets/FLOGA/finetuning data/floga_splits" --teacher-ckpt ckpt/CopernicusFM_ViT_base_varlang_e100.pth --out-ckpt-dir ckpt --epochs 10 --batch-size 4 --min-batch-size 2 --lr 1e-4 --weight-decay 1e-4 --lambda-center 0.0 --lambda-distill 0.0 --lambda-anom 3.0 --anom-margin 1.3 --load-teacher-cache

RUN 4 (after gridsearch)                python3 train_finetune_copernicus_floga_semisupervised.py --data-root ../Datasets/FLOGA --splits-root "../Datasets/FLOGA/finetuning data/floga_splits" --teacher-ckpt ckpt/CopernicusFM_ViT_base_varlang_e100.pth --out-ckpt-dir ckpt --epochs 10 --batch-size 4 --min-batch-size 2 --lr 1e-4 --weight-decay 1e-4 --lambda-center 0.5 --lambda-distill 0.3 --lambda-anom 3.0 --anom-margin 1.3 --load-teacher-cache
